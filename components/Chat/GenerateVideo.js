...
export default function GenerateVideo(props) {
  ...
  // play voideo on click with defined id
  function playVideo() {
    // generateSpeech(text);
    setProcessing(true);
    setIsVideoFinished(false);
    setVideoId("tlk_LbmyMD8W37mrJ0lPBXkYJ");
  }
  ...
  // when it created it will setup the heigh
  useEffect(() => {
  ...
  // Gen video to True start process
  useEffect(() => {
    if (genVideo) {
  ...
  // we get video url user can continue
  useEffect(() => {
    if (videoUrl != null) {
      setProcessing(false);
      updProcessing(false);
